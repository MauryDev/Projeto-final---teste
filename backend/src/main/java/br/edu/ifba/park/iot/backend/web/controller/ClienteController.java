...
import br.edu.ifba.park.iot.backend.jwt.JwtUserDetails;
import br.edu.ifba.park.iot.backend.model.Cliente;
import br.edu.ifba.park.iot.backend.model.dto.cliente.mapper.ClienteMapper;
...
import br.edu.ifba.park.iot.backend.service.ClienteService;
...
public class ClienteController {
  ...
  @Operation(summary = "Cria um novo cliente", description = "Recurso para criar um novo cliente vinculado a um usuário cadastrado. "
      +
      "Requisição exige uso de um bear token. Acesso restrito a Role='CLIENTE'", responses = {
          @ApiResponse(responseCode = "201", description = "Cliente criado com sucesso.", content = @Content(mediaType = "application/json:charset=UTF8-8", schema = @Schema(implementation = UsuarioResponseDto.class))),
          @ApiResponse(responseCode = "409", description = "Cliente CPF já possui cadastro no sistema.", content = @Content(mediaType = "application/json:charset=UTF8-8", schema = @Schema(implementation = ErrorMessage.class))),
          @ApiResponse(responseCode = "422", description = "Recursos não processados por falta de dados ou dados invalidos.", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorMessage.class))),
          @ApiResponse(responseCode = "403", description = "Recursos não permitido ao perfil de ADMIN.", content = @Content(mediaType = "application/json:charset=UTF8-8", schema = @Schema(implementation = ErrorMessage.class)))
      })
  @PostMapping
